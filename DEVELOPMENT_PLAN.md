...
# Phase 5-9: Multi-Model Agent Support (Issue #113)

> **Feature**: Multi-Model Agent Support (Cursor, Aider, Cline, Windsurf, etc.)
> **Issue**: #113
> **Goal**: Abstract the output layer to support multiple agentic coding tools while preserving the core DevPlan methodology.

## Overview

The methodology is the IP — phases, subtasks, verification loops, dual-purpose documentation. The Claude-specific output is just one implementation. This feature adds an adapter layer to support:

| Target | Agent File | Use Case |
|--------|-----------|----------|
| `claude` | CLAUDE.md | Default - Claude Code |
| `cursor` | .cursorrules | Cursor IDE |
| `aider` | .aider.conf.yml | Aider CLI |
| `cline` | .cline/ | VS Code Cline extension |
| `windsurf` | cascade config | Codium Windsurf |
| `generic` | AGENTS.md | Model-agnostic markdown |

---

## Progress Tracking (Phases 5-9)

### Phase 5: Abstraction Layer
- [ ] 5.1.1: Define OutputAdapter interface
- [ ] 5.1.2: Extract Claude-specific code into adapters/claude.ts
- [ ] 5.1.3: Add target parameter to generation tools
- [ ] 5.1.4: Add adapter registry and factory
- [ ] **Phase 5 Tests Pass**

### Phase 6: Cursor Adapter
- [ ] 6.1.1: Research and document .cursorrules format
- [ ] 6.1.2: Implement adapters/cursor.ts
- [ ] 6.1.3: Generate Cursor-optimized phase plans
- [ ] 6.1.4: Add Cursor adapter tests
- [ ] **Phase 6 Tests Pass**

### Phase 7: Aider Adapter
- [ ] 7.1.1: Research and document Aider conventions
- [ ] 7.1.2: Implement adapters/aider.ts
- [ ] 7.1.3: Adapt executor/verifier for Aider architect mode
- [ ] 7.1.4: Add Aider adapter tests
- [ ] **Phase 7 Tests Pass**

### Phase 8: Additional Adapters
- [ ] 8.1.1: Implement adapters/cline.ts
- [ ] 8.1.2: Implement adapters/windsurf.ts
- [ ] 8.1.3: Implement adapters/generic.ts
- [ ] 8.1.4: Add tests for all additional adapters
- [ ] **Phase 8 Tests Pass**

### Phase 9: Documentation & Polish
- [ ] 9.1.1: Update README with multi-target usage
- [ ] 9.1.2: Add adapter contribution guide
- [ ] 9.1.3: Create adapter comparison documentation
- [ ] 9.1.4: Update devplan_start instructions for target selection
- [ ] **Phase 9 Complete**
...
**Goal**: Create the adapter interface and refactor existing Claude code into the first adapter
**Duration**: 2-3 sessions
**Issue**: #113

### Task 5.1: Adapter Foundation

**Git**: Create branch `feature/5-1-adapter-foundation` when starting first subtask.

---

**Subtask 5.1.1: Define OutputAdapter Interface**

**Prerequisites**: None (first subtask of Phase 5)

**Deliverables**:
- [ ] Create `src/adapters/types.ts` with OutputAdapter interface
- [ ] Define all adapter methods with JSDoc
- [ ] Export AdapterTarget type union

**Complete Code**:

Create file `src/adapters/types.ts`:
```typescript
/**
...
export interface OutputAdapter {
   ...
   * Returns null if the target doesn't support separate executor agents.
   */
   ...
   * Returns null if the target doesn't support separate verifier agents.
   */
...
**Success Criteria**:
- [ ] File exists at `src/adapters/types.ts`
- [ ] TypeScript compiles: `npx tsc --noEmit`
- [ ] Interface exported and importable

**Completion Notes**:
- **Implementation**: 
- **Files Created**: src/adapters/types.ts

---

**Subtask 5.1.2: Extract Claude-specific Code into adapters/claude.ts**

**Prerequisites**: 5.1.1 complete

**Deliverables**:
- [ ] Create `src/adapters/claude.ts` implementing OutputAdapter
- [ ] Extract CLAUDE.md generation logic from existing code
- [ ] Extract executor/verifier generation logic
- [ ] Maintain exact same output as current implementation

**Complete Code**:

Create file `src/adapters/claude.ts`:
```typescript
/**
...
export class ClaudeAdapter implements OutputAdapter {
  ...
  generateAgentFile(briefContent: string, config: AdapterConfig): GeneratedFile {
    // Extract existing CLAUDE.md generation logic here
    const content = this.buildClaudeMd(briefContent, config);
    return {
      path: 'CLAUDE.md',
      content,
      isPrimary: true,
  ...
  generateExecutorAgent(briefContent: string, config: AdapterConfig): GeneratedFile | null {
    // Extract existing executor generation logic here
    const content = this.buildExecutorAgent(briefContent, config);
    return {
      path: `.claude/agents/${this.slugify(config.projectName)}-executor.md`,
      content,
    };
  }

  generateVerifierAgent(briefContent: string, config: AdapterConfig): GeneratedFile | null {
    // Extract existing verifier generation logic here  
    const content = this.buildVerifierAgent(briefContent, config);
    return {
      path: `.claude/agents/${this.slugify(config.projectName)}-verifier.md`,
      content,
    };
  }

  transformPhasePlan(planContent: string, _config: AdapterConfig): string {
    // Claude uses the plan as-is
    return planContent;
  }
...
\`\`\``;
  ...
  private buildClaudeMd(briefContent: string, config: AdapterConfig): string {
    // TODO: Extract from existing devplan_generate_claude_md implementation
    // This maintains backward compatibility
    return `# CLAUDE.md\n\n<!-- Generated for ${config.projectName} -->\n\n...`;
  }

  private buildExecutorAgent(briefContent: string, config: AdapterConfig): string {
    // TODO: Extract from existing devplan_generate_executor implementation
    return `# ${config.projectName} Executor Agent\n\n...`;
  }

  private buildVerifierAgent(briefContent: string, config: AdapterConfig): string {
    // TODO: Extract from existing devplan_generate_verifier implementation
    return `# ${config.projectName} Verifier Agent\n\n...`;
  }
}
...
**Success Criteria**:
- [ ] File exists at `src/adapters/claude.ts`
- [ ] Implements OutputAdapter interface
- [ ] TypeScript compiles without errors
- [ ] Existing behavior preserved (same output)

**Completion Notes**:
- **Implementation**: 
- **Files Created**: src/adapters/claude.ts

---

**Subtask 5.1.3: Add Target Parameter to Generation Tools**

**Prerequisites**: 5.1.2 complete

**Deliverables**:
- [ ] Add optional `target` parameter to `devplan_generate_claude_md`
- [ ] Add optional `target` parameter to `devplan_generate_executor`
- [ ] Add optional `target` parameter to `devplan_generate_verifier`
- [ ] Default to 'claude' for backward compatibility
- [ ] Update tool descriptions to mention target options

**Files to Modify**:
- `src/index.ts` - Add target parameter to tool definitions
- `src/handlers/generators.ts` - Update handlers to use adapter

**Success Criteria**:
- [ ] Tools accept `target` parameter
- [ ] Default behavior unchanged (claude)
- [ ] TypeScript compiles
- [ ] Existing tests pass
- [ ] New target parameter documented in tool descriptions
...
**Subtask 5.1.4: Add Adapter Registry and Factory**

**Prerequisites**: 5.1.3 complete

**Deliverables**:
- [ ] Create `src/adapters/index.ts` with registry
- [ ] Implement `getAdapter(target)` factory function
- [ ] Add `listTargets()` function
- [ ] Add `hasNativeAdapter(target)` check
- [ ] Export all adapters from barrel file

**Complete Code**:

Create file `src/adapters/index.ts`:
```typescript
/**
...
const ADAPTERS: Record<AdapterTarget, OutputAdapter> = {
  ...
  cursor: claudeAdapter,   // Placeholder - falls back to Claude until implemented
  aider: claudeAdapter,    // Placeholder
  cline: claudeAdapter,    // Placeholder
  windsurf: claudeAdapter, // Placeholder
  generic: claudeAdapter,  // Placeholder
};
...
/**
 * Check if a target has a dedicated adapter (vs. falling back to Claude).
 */
export function hasNativeAdapter(target: AdapterTarget): boolean {
  // Will expand as adapters are implemented
...
/**
 * List targets that have native adapter implementations.
 */
...
**Success Criteria**:
- [ ] Registry exports all adapters
- [ ] `getAdapter()` returns correct adapter
- [ ] `listTargets()` returns all 6 targets
- [ ] TypeScript compiles
- [ ] All tests pass

**Completion Notes**:
- **Implementation**: 
- **Files Created**: src/adapters/index.ts

---

## Phase 6-9: Implementation Details

Phases 6-9 follow the same pattern as Phase 5. Each adapter phase includes:

1. **Research subtask** - Document target tool's format and best practices
2. **Implementation subtask** - Create adapter implementing OutputAdapter interface  
3. **Plan transformation subtask** - Target-specific plan adjustments if needed
4. **Testing subtask** - Unit + integration tests for the adapter

See Issue #113 for detailed user stories and acceptance criteria for each adapter.

### Phase 6: Cursor Adapter
- Research `.cursorrules` schema
- Implement rules-based instruction format
- Adapt for Cursor Composer workflows

### Phase 7: Aider Adapter  
- Research Aider conventions and architect mode
- Implement `.aider.conf.yml` generation
- Adapt executor/verifier for Aider's workflow

### Phase 8: Additional Adapters
- Cline (VS Code extension format)
- Windsurf (Codium Cascade format)
- Generic (model-agnostic markdown)

### Phase 9: Documentation
- Update README with `--target` flag usage
- Adapter contribution guide for community adapters
- Comparison table of adapter capabilities

---

*Multi-model support phases generated for Issue #113*
