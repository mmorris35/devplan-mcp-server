...
# =============================================================================
# Server Factory (Smithery pattern)
# =============================================================================
...
def create_server() -> FastMCP:
    ...
    All tools, prompts, and resources are defined inside this function
    so they're properly registered when Smithery creates the server.
    """
    server = FastMCP(
...
"""
    ...
    async def devplan_generate_plan(params: GeneratePlanInput) -> str:
        ...
        try:
            ...
            rendered = render_plan(template_name=template_name, **template_vars)
            return rendered
        except Exception as e:
    ...
    async def devplan_update_progress(params: UpdateProgressInput) -> str:
        ...
        next_subtask = re.search(r"\*\*Subtask \d+\.\d+\.\d+:", content[subtask_start + 10:])
        next_task = re.search(r"### Task \d+\.\d+:", content[subtask_start + 10:])
...
Start by reading the development plan."""
    ...
    @server.resource("templates://{name}")
...
def main() -> None:
    ...
    # Create server using the factory function
    server = create_server()

    if args.transport == "http":
        server.run(transport="streamable_http", port=args.port)
    else:
        server.run()
