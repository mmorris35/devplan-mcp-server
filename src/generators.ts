...
export function generatePlan(briefContent: string, lessons?: Lesson[]): string {
	...
	if (phaseTemplates) {
		...
		templateNote =
			...
			`Using language-appropriate scaffold. Feature phases should be enhanced with project-specific details.\n\n`;
...
- [ ] 1.1.2: Add tests for core module`;
	...
	const scaffoldLanguage = templateKey.variant === "static" ? "static" : templateKey.language;
	const featurePhasesSection = generateFeaturesSection(brief, lessons, projectType, scaffoldLanguage);
...
You now have a development plan so detailed that any LLM coding agent can implement it. Each subtask is paint-by-numbers: explicit deliverables, specific files, and testable success criteria.
...
/**
 * Generate a features section that lists MVP features to be structured
 * into implementation phases. Includes relevant lessons if available.
 */
function generateFeaturesSection(brief: ProjectBrief, lessons?: Lesson[], projectType?: string, language?: string): string {
	...
	if (lessons && lessons.length > 0 && projectType) {
		...
		if (relevantLessons.length > 0) {
			...
			if (lessonItems.length > 0) {
				lessonSection = `
> **Lessons Learned** (apply when structuring feature phases):
${lessonItems.join("\n")}
`;
			}
		}
	...
	const techStack = brief.mustUseTech.length > 0
		? `\n**Tech stack**: ${brief.mustUseTech.join(", ")}`
		: "";
...
The following MVP features from PROJECT_BRIEF.md need to be structured into implementation phases:

${featureList}
${lessonSection}${techStack}
